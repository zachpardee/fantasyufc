...
async function main() {
  ...
  try {
    ...
    await client.query(
      `DELETE FROM event_picks WHERE league_id = $1 AND fight_id IN (
    ...
    await client.query(`DELETE FROM matchups WHERE league_id = $1 AND event_id = $2`, [
      LEAGUE_ID,
      BARBOZA_EVENT_ID,
    ]);
    ...
    const barbozaFights = [
      {
        red: F.dellaMaddalena,
        blue: F.covington,
        wc: WC.welterweight,
        isMain: true,
        isCo: false,
        order: 13,
        winner: F.dellaMaddalena,
        outcome: 'ko_tko',
        redOdds: -200,
        blueOdds: 165,
      },
    ...
    function isMethodMatch(picked, outcome) {
      ...
      if (
        picked === 'decision' &&
        ['decision_unanimous', 'decision_split', 'decision_majority'].includes(outcome)
      )
        return true;
      return false;
    }
    ...
    await client.query(
      `DELETE FROM event_picks WHERE league_id=$1 AND fight_id IN ($2,$3,$4,$5,$6,$7)`,
