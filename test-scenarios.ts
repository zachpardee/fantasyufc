...
async function report(leagueId: string, label: string) {
  ...
  console.log(`  Records: ${m.home} ${m.hw}W-${m.hl}L-${m.ht2}T | ${m.away} ${m.aw}W-${m.al}L-${m.at3}T`);
}
...
async function run() {
  ...
  await report(leagueId, 'Scenario 1: m1=4/6 KO (500), m2=2/6 DEC (600) — m2 wins');
  ...
  await db.query(`INSERT INTO roster_fighters (roster_id,fighter_id,slot_type) VALUES ($1,$2,'starter') ON CONFLICT DO NOTHING`, [m1Roster.id, top6[0].red_fighter_id]);
  ...
  await finalizeMatchupResults(leagueId, eventId);
  await report(leagueId, 'Scenario 2: both 5/6+DEC (1700), m1 roster win +50 → 1750, m1 wins');
