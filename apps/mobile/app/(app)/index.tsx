        ...
        <View style={[styles.eventCard, liveEvent && styles.eventCardLive]}>
          ...
          {((upcomingEvent as any).venue || (upcomingEvent as any).location) && (
            <Text style={styles.eventVenue} numberOfLines={1}>
              {[(upcomingEvent as any).venue, (upcomingEvent as any).location].filter(Boolean).join(' · ')}
            </Text>
          )}
      ...
      <View style={styles.section}>
        ...
        {leagues?.map((league) => {
          ...
          const statusColors: Record<string, string> = {
            active: '#1a3a1a', playoffs: '#1a1a3a', completed: '#2a2a2a', setup: '#2a1a1a',
          };
          return (
            <TouchableOpacity
              key={league.id}
              style={[styles.leagueCard, { borderLeftColor: statusColors[league.status] ?? '#2a2a2a' }]}
            ...
            >
              ...
              <View style={[styles.statusBadge, { backgroundColor: statusColors[league.status] ?? '#222' }]}>
...
const styles = StyleSheet.create({
  ...
  eventCard: {
    margin: 16, marginTop: 0, backgroundColor: '#141414',
    borderRadius: 12, padding: 20, borderWidth: 1, borderColor: '#2a2a2a',
  ...
  button: { backgroundColor: '#c8102e', borderRadius: 8, paddingHorizontal: 24, paddingVertical: 13, alignItems: 'center' },
  ...
  leagueCard: {
    ...
    borderWidth: 1, borderColor: '#222', flexDirection: 'row', alignItems: 'center',
    borderLeftWidth: 4,
  },
  ...
  stakingBadge: { backgroundColor: '#1a1000', borderRadius: 4, paddingHorizontal: 6, paddingVertical: 2 },
