...
export default function FighterDetailScreen() {
  ...
  if (!fighter) {
    return <View style={s.center}><Text style={s.empty}>Fighter not found</Text></View>;
  }
    ...
    <ScrollView style={s.container}>
      ...
      <View style={s.hero}>
        ...
        <View style={s.heroOverlay}>
          {fighter.isChampion && (
            <View style={s.champBadge}><Text style={s.champBadgeText}>CHAMPION</Text></View>
          )}
          {!fighter.isChampion && fighter.ranking && (
            <View style={s.rankBadge}><Text style={s.rankBadgeText}>#{fighter.ranking}</Text></View>
          )}
          <Text style={s.heroName}>{fighter.firstName} {fighter.lastName}</Text>
      ...
      <View style={s.statsGrid}>
        <StatCard label="Avg Fantasy Pts" value={fighter.averageFantasyPoints?.toFixed(1) ?? '--'} />
      ...
      {history.length > 0 && (
        <View style={s.historySection}>
          ...
          {history.map((fight: any, i: number) => {
            ...
            const resultColor = isPending ? '#555' : isDraw ? '#ffd700' : isWin ? '#4caf50' : '#ff5252';
            const resultLabel = isPending ? 'Upcoming' : isDraw ? fmtOutcome(fight.outcome) : isWin ? `W · ${fmtOutcome(fight.outcome)}` : `L · ${fmtOutcome(fight.outcome)}`;
            return (
              <View key={`${fight.fightId}-${i}`} style={s.historyRow}>
                ...
                <View style={s.historyInfo}>
                  <Text style={s.historyEvent} numberOfLines={1}>{fight.eventName}</Text>
                  <Text style={s.historyOutcome}>{resultLabel}</Text>
                </View>
                <Text style={s.historyDate}>
                  {fight.scheduledAt ? new Date(fight.scheduledAt).toLocaleDateString('en-US', { month: 'short', year: 'numeric' }) : ''}
                </Text>
              </View>
            );
          })}
...
const s = StyleSheet.create({
  ...
  rankBadge: {
    alignSelf: 'flex-start', backgroundColor: '#c8102e',
    borderRadius: 4, paddingHorizontal: 8, paddingVertical: 3, marginBottom: 8,
  },
  rankBadgeText: { color: '#fff', fontSize: 10, fontWeight: '800' },
  heroName: { color: '#fff', fontSize: 26, fontWeight: '800', textShadowColor: '#000', textShadowRadius: 8 },
  heroNickname: { color: '#aaa', fontSize: 14, marginTop: 2, textShadowColor: '#000', textShadowRadius: 6 },
  heroWeightClass: { color: '#c8102e', fontSize: 12, fontWeight: '700', letterSpacing: 0.5, marginTop: 4 },
  ...
  statBoxLabel: { color: '#555', fontSize: 10, fontWeight: '700', letterSpacing: 0.5, marginTop: 2 },
  divider: { width: 1, backgroundColor: '#1e1e1e', marginVertical: 12 },

  statsGrid: { flexDirection: 'row', flexWrap: 'wrap', padding: 8, gap: 8 },
  statCard: {
    flex: 1, minWidth: '45%', backgroundColor: '#111', borderRadius: 8,
  ...
  sectionTitle: { color: '#444', fontSize: 11, fontWeight: '700', letterSpacing: 1, marginBottom: 12 },
  historyRow: {
    flexDirection: 'row', alignItems: 'center', gap: 12,
    paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: '#111',
  },
  historyResult: { width: 32, height: 32, borderRadius: 8, alignItems: 'center', justifyContent: 'center' },
