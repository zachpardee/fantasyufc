...
async function main() {
  const { rows: [event] } = await db.query(`SELECT id FROM ufc_events WHERE status = 'live'`);
  ...
  for (const f of fights) {
    console.log(`  [${f.ufc_fight_id}] ${f.red} vs ${f.blue} | status: ${f.status} | result: ${f.outcome ?? 'none'}`);
  }
  ...
  for (const p of pending) {
    console.log(`  fight_id: ${p.fight_id} | ufc_fight_id: ${p.ufc_fight_id} | ${p.red_corner} vs ${p.blue_corner} | bet on: ${p.fighter_bet_on}`);
  }
