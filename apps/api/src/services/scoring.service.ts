...
export async function refreshStakingMatchupScores(leagueId: string, eventId: string) {
  // Matchup score is money-on-hand for the event: start from the weekly budget ($100),
  // subtract every stake placed, and add back the payout of each settled (won) bet.
  // Whoever ends the event with the most money wins the matchup.
  const {
    rows: [lg],
  } = await db.query(`SELECT COALESCE(weekly_budget, 100) AS budget FROM leagues WHERE id = $1`, [
    leagueId,
  ]);
  const budget = parseFloat(lg?.budget ?? 100);
    ...
    await db.query(
      ...
      UPDATE matchups SET
        home_score = (
          ...
          - COALESCE((SELECT SUM(p.stake) FROM staking_parlays p WHERE p.event_id=$2 AND p.member_id=$3), 0)
          + COALESCE((SELECT SUM(s.actual_payout) FROM staking_singles s WHERE s.event_id=$2 AND s.member_id=$3 AND s.status != 'pending'), 0)
          + COALESCE((SELECT SUM(p.actual_payout) FROM staking_parlays p WHERE p.event_id=$2 AND p.member_id=$3 AND p.status != 'pending'), 0)
        ),
        away_score = (
          $5
          - COALESCE((SELECT SUM(s.stake) FROM staking_singles s WHERE s.event_id=$2 AND s.member_id=$4), 0)
          - COALESCE((SELECT SUM(p.stake) FROM staking_parlays p WHERE p.event_id=$2 AND p.member_id=$4), 0)
          + COALESCE((SELECT SUM(s.actual_payout) FROM staking_singles s WHERE s.event_id=$2 AND s.member_id=$4 AND s.status != 'pending'), 0)
          + COALESCE((SELECT SUM(p.actual_payout) FROM staking_parlays p WHERE p.event_id=$2 AND p.member_id=$4 AND p.status != 'pending'), 0)
        )
      WHERE id = $1
    `,
      [m.id, eventId, m.home_team_id, m.away_team_id, budget],
    );
  }
}

export async function processStakingFightResult(
  fightId: string,
  ...
  try {
    ...
    const {
      rows: [fightRow],
    } = await client.query(`SELECT event_id FROM fights WHERE id = $1`, [fightId]);
    if (!fightRow) {
    ...
    for (const leg of legs) {
      if (isVoid) {
        ...
        await client.query(`UPDATE staking_parlay_legs SET result = 'void' WHERE id = $1`, [
          leg.id,
        ]);
      } else {
        const result = winnerId ? (leg.fighter_id === winnerId ? 'won' : 'lost') : 'lost';
        await client.query(`UPDATE staking_parlay_legs SET result = $1 WHERE id = $2`, [
          result,
          leg.id,
        ]);
    ...
    for (const parlay of pendingParlays) {
      ...
      if (wonLegs.length === 0) {
        ...
        // An unpriced winning leg (no posted line) contributes 1.0 rather than NaN.
        const effectiveOdds = wonLegs.reduce(
          (acc: number, leg: any) =>
            acc * (leg.decimal_odds != null ? parseFloat(leg.decimal_odds) || 1 : 1),
          1,
        );
    ...
    const affectedLeagueIds = [
      ...new Set([...singles.map((s: any) => s.league_id), ...legs.map((l: any) => l.league_id)]),
    ];
...
export async function processFightResult(fightResultId: string) {
  ...
  try {
    ...
    if (fightResult.winner_id && winnerOdds == null) {
      console.warn(
        `[Scoring] No odds on file for fight ${fightResult.fight_id} — underdog bonus skipped`,
      );
    } else if (isUnderdog) {
      console.log(
        `[Scoring] Underdog bonus triggered for fight ${fightResult.fight_id} (odds: +${winnerOdds})`,
      );
    }
    ...
    for (const lc of leagueConfigs) {
      ...
      if (fightResult.winner_id) {
        ...
        const ptsMethod: number =
          fightResult.outcome === 'ko_tko'
            ? lc.pts_ko_tko
            : fightResult.outcome === 'submission'
              ? lc.pts_submission
              : lc.pts_decision;
  ...
  const {
    ...
    `SELECT fr.fight_id, fr.winner_id, fr.outcome FROM fight_results fr WHERE fr.id = $1`,
    [fightResultId],
  );
