...
async function run() {
  ...
  const { rows: leagues } = await db.query(`
    ...
    LIMIT 5
  `);
  console.log('Leagues:', leagues.map((l: any) => `${l.name} (${l.member_count} members, ${l.status})`));

  const league = leagues[0];
  if (!league) { console.log('No leagues found'); return; }
  ...
  `, [league.id]);
  console.log('Members:', members.map((m: any) => m.team_name));
  ...
  `, [league.id]);
  console.log('Past events:', events.map((e: any) => e.name));
  ...
  for (const event of events) {
    ...
    for (let i = 0; i < members.length; i++) {
      ...
      inserted++;
      console.log(`  ${event.name} · ${member.team_name} → ${pick.name} (${pointsEarned > 0 ? '+30' : '0 pts'})`);
    }
  }
