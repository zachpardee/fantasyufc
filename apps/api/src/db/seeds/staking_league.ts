...
async function run() {
  ...
  try {
    ...
    if (existing.length) {
      ...
      await client.query(`DELETE FROM staking_parlay_legs WHERE parlay_id IN (SELECT id FROM staking_parlays WHERE league_id = $1)`, [id]);
    ...
    `, [
      leagueId, EVENT_ID,
      memberIds['Thunder Bets'],  memberIds['Yellow Flash'],
      memberIds['Red Rockets'],   memberIds['Green Gamblers'],
    ]);
    ...
    async function placeSingle(memberId: string, fightId: string, fighterId: string, stake: number) {
    ...
    `, [pRow.id, pavFight, byId[pavFight].red_fighter_id, -300, pavDec,
                songFight, byId[songFight].red_fighter_id, -175, songDec]);
    await client.query(
      `UPDATE league_members SET staking_balance = staking_balance - 10 WHERE id = $1`, [thunderId],
    );
    console.log(`Thunder Bets: Pavlovich $20 + Song $15 + parlay $10 (dec: ${parlayDec.toFixed(3)}x → $${parlayPayout})`);
    ...
    const { rows: balances } = await db.query(
      `SELECT team_name, staking_balance FROM league_members WHERE league_id = $1 ORDER BY team_name`, [leagueId],
    );
    console.log('\nBalances:');
