...
waiversRouter.post('/', requireAuth, async (req: AuthRequest, res, next) => {
  try {
    ...
    const {
      rows: [fighter],
    } = await db.query(`SELECT id, first_name, last_name FROM fighters WHERE id = $1`, [
      body.fighterId,
    ]);
    ...
    if (priority === undefined) {
      const {
        rows: [{ max_priority }],
      } = await db.query<{ max_priority: string | null }>(
        `
  ...
  try {
    ...
    try {
      ...
      for (let i = 0; i < claimIds.length; i++) {
        await client.query(`UPDATE waiver_claims SET priority = $1 WHERE id = $2`, [
          i + 1,
          claimIds[i],
        ]);
      }
