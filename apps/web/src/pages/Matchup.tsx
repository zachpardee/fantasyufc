...
export function MatchupPage() {
  ...
  const { data: homePicks, isLoading: homePicksLoading } = useQuery<any>({
    ...
    queryFn: () =>
      apiClient.get(
        `/leagues/${leagueId}/picks/${matchup!.eventId}?memberId=${matchup!.homeTeamId}`,
      ),
  ...
  const { data: awayPicks, isLoading: awayPicksLoading } = useQuery<any>({
    ...
    queryFn: () =>
      apiClient.get(
        `/leagues/${leagueId}/picks/${matchup!.eventId}?memberId=${matchup!.awayTeamId}`,
      ),
  ...
  const { data: homeChampion } = useQuery<any>({
    ...
    queryFn: () =>
      apiClient.get(
        `/leagues/${leagueId}/picks/${matchup!.eventId}/champion?memberId=${matchup!.homeTeamId}`,
      ),
    enabled: !!matchup?.eventId && !!matchup?.homeTeamId,
    refetchInterval: liveRefetchInterval,
  });
  ...
  const { data: awayChampion } = useQuery<any>({
    ...
    queryFn: () =>
      apiClient.get(
        `/leagues/${leagueId}/picks/${matchup!.eventId}/champion?memberId=${matchup!.awayTeamId}`,
      ),
    enabled: !!matchup?.eventId && !!matchup?.awayTeamId,
    refetchInterval: liveRefetchInterval,
  });
  ...
  const { data: homeStaking } = useQuery<any>({
    ...
    queryFn: () =>
      apiClient.get(
        `/leagues/${leagueId}/staking/${matchup!.eventId}?memberId=${matchup!.homeTeamId}`,
      ),
  ...
  const { data: awayStaking } = useQuery<any>({
    ...
    queryFn: () =>
      apiClient.get(
        `/leagues/${leagueId}/staking/${matchup!.eventId}?memberId=${matchup!.awayTeamId}`,
      ),
  ...
  function calcStakingScore(staking: any): number {
    ...
    const pendingStake = allBets
      ...
      .reduce((s: number, b: any) => s + (parseFloat(b.stake) || 0), 0);
    ...
    <div style={styles.page}>
      ...
      {seasonEvents.length > 0 && (
        <div ref={stripRef} style={styles.historyStrip}>
          {seasonEvents.map((ev) => {
            ...
            const isBrowsingThisEvent =
              !myM && eventMatchups.some((m) => m.id === browsingMatchupId);
            ...
            const isActive =
              (!!selectedMatchupId && myM?.id === selectedMatchupId) ||
              selectedEventId === ev.eventId ||
              isBrowsingThisEvent ||
              (isViewingDefault && isCurrentEvent);
              ...
              <button
                ...
                style={{
                  ...
                  ...(isActive
                    ...
                    : isCurrentEvent
                      ? styles.historyChipCurrent
                      : {}),
                  ...(!myM && !hasAnyMatchups && !isActive ? styles.historyChipNoMatchup : {}),
                }}
                onClick={() => {
                  setShowMatchupPicker(false);
                  if (myM) {
                  ...
                  hasScore ? (
                    <>
                      ...
                      <span style={styles.chipScore}>
                        {isStaking ? fmtChipScore(myScore!) : myScore!.toFixed(0)}–
                        {isStaking ? fmtChipScore(oppScore!) : oppScore!.toFixed(0)}
                      </span>
                      {(isWin || isLoss) && (
                        <span
      ...
      {!matchup && selectedEventId ? (
        (() => {
          ...
          const isPlayoff =
            ev?.eventId === league?.playoffSemisEventId ||
            ev?.eventId === league?.playoffFinalsEventId;
          return (
            <div style={styles.empty}>
              <div style={{ marginBottom: 12 }}>
                {isPlayoff ? (
                  <Trophy size={28} color="#555" />
                ) : (
              ...
              <div style={{ color: '#444', fontSize: 13 }}>
                {isPlayoff
                  ? 'Playoff matchup will be set once the regular season ends.'
                  : 'Matchup not yet assigned for this event.'}
              </div>
            </div>
          );
        })()
        ...
        <div style={styles.empty}>
          ...
          <div style={{ color: '#555', fontSize: 14 }}>
            Matchups are generated when the season schedule is set. Check back after the
            commissioner starts the season.
          </div>
        </div>
      ) : (
        <>
          <div style={styles.eventHeader}>
            ...
            {matchup.scheduledAt && (
              <div style={styles.eventDate}>
                {new Date(matchup.scheduledAt).toLocaleDateString('en-US', {
                  weekday: 'short',
                  month: 'short',
                  day: 'numeric',
                  year: 'numeric',
            ...
            return (
              <div style={{ ...styles.scoreboard, ...(isMobile ? styles.scoreboardMobile : {}) }}>
                <div style={styles.teamBlock}>
                  <div style={styles.teamLabelRow}>
                    ...
                    >
                      <MemberAvatar
                        ...
                        color={leftColor}
                        size={isMobile ? 24 : 32}
                        avatarUrl={leftMember?.avatarUrl}
                        onClick={() => leftMember && setSelectedMember(leftMember)}
                      />
                      ...
                      {leftHasBmf && (
                        <BeltHalo
                          size={isMobile ? 24 : 32}
                          variant="bmf"
                          position={leftHasBelt ? 'bottom' : 'top'}
                        />
                      )}
                    </div>
                  ...
                  >
                    {isStaking ? fmtStakeScore(leftStakingScore) : leftPicksScore.toFixed(0)}
                  </div>
                  <div style={styles.scoreUnit}>{isStaking ? 'bankroll' : 'matchup pts'}</div>
                </div>

                <div
                  style={{ ...styles.vsBlock, flex: isMobile ? '0 0 48px' : 1, cursor: 'pointer' }}
                  onClick={() => setShowMatchupPicker((v) => !v)}
                >
                ...
                <div style={{ ...styles.teamBlock, alignItems: 'flex-end' }}>
                  <div style={{ ...styles.teamLabelRow, flexDirection: 'row-reverse' }}>
                    ...
                    >
                      <MemberAvatar
                        ...
                        color={rightColor}
                        size={isMobile ? 24 : 32}
                        avatarUrl={rightMember?.avatarUrl}
                        onClick={() => rightMember && setSelectedMember(rightMember)}
                      />
                      ...
                      {rightHasBmf && (
                        <BeltHalo
                          size={isMobile ? 24 : 32}
                          variant="bmf"
                          position={rightHasBelt ? 'bottom' : 'top'}
                        />
                      )}
                    </div>
                  ...
                  >
                    {isStaking ? fmtStakeScore(rightStakingScore) : rightPicksScore.toFixed(0)}
                  </div>
                  <div style={styles.scoreUnit}>{isStaking ? 'bankroll' : 'matchup pts'}</div>
                </div>
              </div>
            ...
            <div style={styles.pickerOverlay} onClick={() => setShowMatchupPicker(false)}>
              <div style={styles.pickerSheet} onClick={(e) => e.stopPropagation()}>
                ...
                {eventMatchups.map((m) => {
                  ...
                  const isActive =
                    browsingMatchupId === m.id || (!browsingMatchupId && isMyMatchup);
                  return (
                    <button
                      key={m.id}
...
function SeasonTable({
  ...
  const rows: SeasonRow[] = relevantEvents
    .map(
      (ev): SeasonRow => ({
  ...
  return (
    <div style={styles.seasonSection}>
      <button style={styles.sectionHeaderBtn} onClick={() => setOpen((v) => !v)}>
        <span style={styles.sectionTitle}>
          SEASON BREAKDOWN <span style={styles.collapseChevron}>{open ? '▲' : '▼'}</span>
        </span>
      </button>
      {open && (
        <div style={styles.seasonTable}>
          <div style={styles.seasonHeaderRow}>
            ...
            <div style={{ ...styles.seasonTeamCell, justifyContent: 'flex-start' }}>
              {awayTeamName}
            </div>
          </div>
          {rows.map((r) => {
            const short = r.eventName
              .replace(/^UFC\s+Fight\s+Night:\s*/i, 'FN: ')
              .replace(/^UFC\s+/i, 'UFC ');
            ...
            return (
              <div
                key={r.eventId}
                style={{ ...styles.seasonRow, ...(r.isCurrent ? styles.seasonRowCurrent : {}) }}
              >
                  ...
                  <span
                    style={{
                      ...
                      ...(isStaking
                        ? {
                            color:
                              r.homeScore < 0 ? '#ff5252' : r.homeScore > 0 ? '#4caf50' : '#555',
                          }
                        : {}),
                    }}
                  >
                ...
                <div style={{ ...styles.seasonEventCell, textAlign: 'center' as const }}>
                  {short}
                </div>
                  ...
                  <span
                    style={{
                      ...
                      ...(isStaking
                        ? {
                            color:
                              r.awayScore < 0 ? '#ff5252' : r.awayScore > 0 ? '#4caf50' : '#555',
                          }
                        : {}),
                    }}
                  >
          ...
          <div style={styles.seasonTotalRow}>
            <div style={styles.seasonScoreCell}>
              <span
                style={{
                  ...styles.seasonTotalPts,
                  ...(isStaking ? { color: totalHome < 0 ? '#ff5252' : '#ff8c42' } : {}),
                }}
              >
            ...
            <div style={{ ...styles.seasonScoreCell, justifyContent: 'flex-start' }}>
              <span
                style={{
                  ...styles.seasonTotalPts,
                  ...(isStaking ? { color: totalAway < 0 ? '#ff5252' : '#ff8c42' } : {}),
                }}
              >
      ...
      {hasScores ? (
        <>
          {picks.map((p) => {
            ...
            const fullName: string = isRed
              ? `${p.redFirstName} ${p.redLastName}`
              : `${p.blueFirstName} ${p.blueLastName}`;
            const earned = +(p.pointsEarned ?? 0);
            const isDecision = [
              'decision_unanimous',
              'decision_split',
              'decision_majority',
            ].includes(p.resultOutcome);
            const methodMatch =
            ...
            return (
              <div key={p.id ?? fullName} style={styles.bdPickRow}>
                <span
                  style={{
                    ...styles.bdFighter,
                    color: p.isCorrect ? '#ccc' : p.isCorrect === false ? '#333' : '#666',
                  }}
                >
                  {fullName}
                </span>
          ...
          {(sweepBonus > 0 || championPts > 0) && <div style={styles.breakdownDividerRow} />}
          {sweepBonus > 0 && (
