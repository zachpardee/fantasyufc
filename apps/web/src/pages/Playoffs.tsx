...
function fmtBalance(
  n: number,
  weeklyBudget: number,
): { display: string; pnl: string; pnlPositive: boolean } {
    ...
    <div style={styles.matchupCard}>
      ...
      <div style={styles.matchupRow}>
        ...
        <div style={{ ...styles.teamSide, ...(homeWon && scored ? styles.winnerSide : {}) }}>
          ...
          <span style={styles.teamRecord}>
            {matchup.homeWins}–{matchup.homeLosses}
          </span>
          {scored ? (
            isStaking ? (
              <div style={styles.stakingScoreCol}>
                <span style={{ ...styles.score, ...(homeWon ? styles.winnerScore : {}) }}>
                  {homeStaking!.display}
                </span>
                <span
                  style={{
                    ...styles.pnlBadge,
                    color: homeStaking!.pnlPositive ? '#4caf50' : '#ff5252',
                  }}
                >
                  {homeStaking!.pnl}
                </span>
        ...
        <div
          style={{
            ...styles.teamSide,
            alignItems: 'flex-end',
            ...(awayWon && scored ? styles.winnerSide : {}),
          }}
        >
          ...
          <span
            style={{
              ...styles.teamName,
              textAlign: 'right',
              ...(awayWon && scored ? styles.winnerName : {}),
            }}
          >
            {matchup.awayTeamName}
          </span>
          <span style={{ ...styles.teamRecord, textAlign: 'right' as const }}>
            {matchup.awayWins}–{matchup.awayLosses}
          </span>
          {scored ? (
            isStaking ? (
              <div style={{ ...styles.stakingScoreCol, alignItems: 'flex-end' }}>
                <span style={{ ...styles.score, ...(awayWon ? styles.winnerScore : {}) }}>
                  {awayStaking!.display}
                </span>
                <span
                  style={{
                    ...styles.pnlBadge,
                    color: awayStaking!.pnlPositive ? '#4caf50' : '#ff5252',
                  }}
                >
                  {awayStaking!.pnl}
                </span>
...
function PreviewCard({
  ...
  top: { seed: number; name: string };
  bottom: { seed: number; name: string };
}) {
  return (
      ...
      {phase === 'none' && seeds.length > 0 && (
        <div style={styles.section}>
          ...
          <p style={styles.previewNote}>
            Seeding is based on current standings. Dashed borders indicate projected matchups.
          </p>
          {seeds.length >= 4 ? (
            <div style={{ ...styles.bracket, ...(isMobile ? styles.bracketMobile : {}) }}>
              <div style={styles.bracketCol}>
                ...
                <PreviewCard
                  top={{ seed: 1, name: seeds[0].teamName }}
                  bottom={{ seed: 4, name: seeds[3].teamName }}
                />
                <PreviewCard
                  top={{ seed: 2, name: seeds[1].teamName }}
                  bottom={{ seed: 3, name: seeds[2].teamName }}
                />
              </div>
            ...
            <div style={styles.bracketSingle}>
              ...
              <PreviewCard
                top={{ seed: 1, name: seeds[0].teamName }}
                bottom={{ seed: 2, name: seeds[1].teamName }}
              />
            </div>
          ) : null}
        </div>
