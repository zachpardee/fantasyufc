  ...
  if (league && !isCommissioner) {
    return (
      <div style={styles.page}>
        <div style={styles.empty}>Commissioner access only.</div>
      </div>
    );
  }
  ...
  return (
    <section style={styles.section}>
      ...
      <p style={{ ...styles.hint, marginBottom: 16 }}>
        Enter American-style moneyline odds (e.g. -250 or +180). You can also auto-sync from The
        Odds API if ODDS_API_KEY is configured.
      </p>
      ...
      {fights.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 }}>
          {[...fights]
            .sort((a, b) => (b.boutOrder ?? 0) - (a.boutOrder ?? 0))
            .map((fight) => {
              ...
              return (
                <div key={fight.id} style={oddsRowStyle}>
                  ...
                  <input
                    ...
                    value={entry.red}
                    onChange={(e) =>
                      setOddsMap((m) => ({ ...m, [fight.id]: { ...entry, red: e.target.value } }))
                    }
                  />
                  ...
                  <input
                    ...
                    value={entry.blue}
                    onChange={(e) =>
                      setOddsMap((m) => ({ ...m, [fight.id]: { ...entry, blue: e.target.value } }))
                    }
                  />
...
const oddsName: React.CSSProperties = {
  color: '#ccc',
  fontSize: 13,
  fontWeight: 600,
  whiteSpace: 'nowrap',
...
const oddsInput: React.CSSProperties = {
  background: '#111',
...
function SettingsSection({ league, leagueId, qc }: { league: any; leagueId: string; qc: any }) {
  ...
  const mutation = useMutation({
    mutationFn: () =>
      apiClient.patch(`/leagues/${leagueId}`, {
        name,
        maxTeams: +maxTeams,
      }),
    onSuccess: () => {
  ...
  return (
    <section style={styles.section}>
      ...
      <div style={styles.saveRow}>
        {saved && <span style={styles.savedMsg}>Saved!</span>}
        {mutation.isError && (
          <span style={styles.errMsg}>{(mutation.error as any)?.error ?? 'Failed to save'}</span>
        )}
        <button
          style={styles.saveBtn}
  ...
  return (
    <section style={styles.section}>
      ...
      <div style={styles.actionRow}>
        <div>
          ...
          <div style={styles.actionHint}>
            Rebuilds all future matchups using a balanced round-robin. Completed matchups are
            preserved.
          </div>
        </div>
        <button
      ...
      {regenMutation.isError && (
        <p style={styles.errMsg}>{(regenMutation.error as any)?.error ?? 'Failed'}</p>
      )}

      {canStartPlayoffs && (
        <div style={{ marginTop: 20 }}>
          <div style={styles.actionLabel}>Start Playoffs</div>
          <div style={styles.actionHint}>Semifinals: {semisEvent ? `${semisEvent.name}` : '—'}</div>
          ...
          <div style={{ ...styles.seedTable, marginTop: 10 }}>
            ...
            {seeded.map((m, i) => (
              <div
                key={m.id}
                style={{ ...styles.seedRow, ...(i === 3 ? { borderBottom: 'none' } : {}) }}
              >
          ...
          {!confirmStart ? (
            ...
            <div style={{ ...styles.confirmBox, marginTop: 12 }}>
              ...
              <div style={styles.confirmRow}>
                <button style={styles.cancelBtn} onClick={() => setConfirmStart(false)}>
                  Cancel
                </button>
                <button
          ...
          {canAdvance && (
            <div style={styles.advanceBlock}>
              {advanceMutation.isError && (
                <p style={styles.errMsg}>
                  {(advanceMutation.error as any)?.error ?? 'Failed to advance'}
                </p>
              )}
              <button
                style={styles.saveBtn}
...
function MatchupRow({ matchup, isStaking = false }: { matchup: any; isStaking?: boolean }) {
  ...
  return (
    <div style={styles.matchupRow}>
      ...
      <span style={styles.mScore}>
        {fmtMatchupScore(home, isStaking)} – {fmtMatchupScore(away, isStaking)}
      </span>
      <span style={{ ...styles.mTeam, textAlign: 'right', ...(awayWon ? styles.mWinner : {}) }}>
        {matchup.awayTeamName}
      </span>
      {matchup.eventName && <span style={styles.mEvent}>{matchup.eventName}</span>}
    </div>
  );
}
    ...
    <section style={{ ...styles.section, borderColor: '#3a1a1a' }}>
      ...
      {!confirming ? (
        ...
        <div style={styles.confirmBox}>
          <p style={styles.confirmText}>
            This will permanently delete the league and all its data. Are you sure?
          </p>
          <div style={styles.confirmRow}>
          ...
          {deleteMutation.isError && (
            <p style={styles.errMsg}>
              {(deleteMutation.error as any)?.error ?? 'Failed to delete'}
            </p>
          )}
        </div>
      )}
