      ...
      <div style={styles.header}>
        <div>
          ...
          <div style={styles.eventMeta}>
            {new Date(currentEvent.scheduledAt ?? currentEvent.scheduled_at).toLocaleDateString(
              'en-US',
              {
                weekday: 'long',
                month: 'long',
      ...
      {showSummary || locked ? (
        <div style={styles.summaryWrap}>
            ...
            <thead>
              <tr>
                {['Fight', 'Your Pick', 'Method', ...(isLive ? ['Result'] : [])].map((h) => (
                  <th key={h} style={styles.summaryTh}>
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {fights.map((fight, i) => {
                ...
                return (
                  <tr
                    key={fight.id}
                    style={i % 2 === 0 ? styles.summaryRowEven : styles.summaryRowOdd}
                  >
                    ...
                    <td style={styles.summaryTd}>
                      {pickedName ? (
                        <div style={styles.summaryPickCell}>
                          ...
                          <span
                            style={{ color: pickedRed ? '#e05555' : '#5599dd', fontWeight: 600 }}
                          >
                            {pickedName}
                          </span>
                        </div>
                    ...
                    <td style={styles.summaryTd}>
                      {method ? (
                        <span style={styles.methodBadge}>{methodLabel[method] ?? method}</span>
                      ) : (
                        <span style={{ color: '#333' }}>—</span>
                      )}
                    </td>
                      ...
                      <td style={styles.summaryTd}>
                        {isCorrect === true && (
                          <span style={styles.correctBadge}>
                            ✓ +{(+fight.pointsEarned).toFixed(0)} pts
                          </span>
                        )}
                        {isWrong && <span style={styles.wrongBadge}>✗ 0 pts</span>}
                        {isCorrect === null && pickedId && (
                          <span style={{ color: '#555', fontSize: 12 }}>Pending</span>
                        )}
          ...
          <div style={styles.summaryFooter}>
            {!locked && (
              <button style={styles.editPicksFooterBtn} onClick={() => setShowSummary(false)}>
                Edit Picks
              </button>
            )}
            {(isCommissioner || locked) && (
              <Link to={`/league/${leagueId}/picks/comparison`} style={styles.compareLink}>
                Compare All Picks →
              </Link>
            )}
          </div>
          ...
          {fights.length > 0 && (
            <div style={styles.champSection}>
              <div style={styles.champSectionHeader}>
                ...
                <span style={styles.champSectionSub}>
                  Pick one fighter from the card — +30 pts if they win
                </span>
              </div>
          ...
          <div style={styles.footer}>
            <div style={styles.publicToggleRow}>
              <label style={styles.publicToggleLabel}>
                ...
                >
                  <div
                    style={{ ...styles.toggleThumb, ...(picksPublic ? styles.toggleThumbOn : {}) }}
                  />
                </div>
                <span style={styles.publicToggleText}>Share picks publicly</span>
              </label>
              {picksPublic && (
                <span style={styles.publicWarning}>
                  Opponents can see your picks before the event locks
                </span>
              )}
            </div>
...
function FighterPick({
  ...
  return (
    ...
    >
      ...
      {isPicked && isCorrect === null && !locked && <div style={styles.pickedTag}>YOUR PICK</div>}
    </button>
  );
}
