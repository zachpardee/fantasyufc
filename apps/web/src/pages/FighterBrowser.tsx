...
export function FighterBrowserPage() {
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={{ padding: '0 24px' }}><table style={styles.table}>
        <thead>
            ...
            <tr key={f.id} style={styles.row}>
              ...
              <td style={styles.td}>
                <div style={styles.nameRow}>
                  {(f as any).imageUrl && (
                    <div
                      style={{ width: 36, height: 40, borderRadius: 4, overflow: 'hidden', flexShrink: 0, background: '#222', cursor: 'zoom-in' }}
                      onClick={() => setZoomedImage({ url: (f as any).imageUrl, name: `${f.firstName} ${f.lastName}` })}
                    >
...
const styles: Record<string, React.CSSProperties> = {
  ...
  lightboxOverlay: { position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'zoom-out' },
  lightboxImg: { maxHeight: '85vh', maxWidth: '90vw', borderRadius: 8, objectFit: 'contain', objectPosition: 'top center', boxShadow: '0 8px 40px rgba(0,0,0,0.6)' },
};
