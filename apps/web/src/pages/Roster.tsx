        ...
        {!isEmpty && !isLoading && (
          <div style={styles.section}>
            <h2 style={styles.sectionTitle}>
              Roster <span style={styles.count}>{all.length}</span>
            </h2>
            {all.map((f) => (
              <FighterRow key={f.id} fighter={f} />
            ))}
          </div>
        )}
      </div>
    ...
    <div style={styles.row}>
      <div style={styles.rowLeft}>
        ...
        <div>
          ...
          <div style={styles.meta}>
            {fighter.weightClassName} · via {fighter.acquiredVia}
          </div>
          <div style={styles.nextEvent}>
            {fighter.nextEventName ? (
              <>
                {fighter.nextEventName} ·{' '}
                {new Date(fighter.nextEventDate).toLocaleDateString('en-US', {
                  month: 'short',
                  day: 'numeric',
                })}
              </>
      ...
      <div style={styles.rowRight}>
        <span style={styles.avgPts}>
          {fighter.averageFantasyPoints != null ? (+fighter.averageFantasyPoints).toFixed(1) : '--'}{' '}
          avg pts
        </span>
      </div>
    </div>
