...
async function run() {
  ...
  for (const [fightId, , side, outcome, round] of fights) {
    const {
      rows: [fight],
    } = await db.query(`SELECT red_fighter_id, blue_fighter_id FROM fights WHERE id=$1`, [fightId]);
  ...
  console.log('\nPick summary:');
  picks.rows.forEach((r: any) =>
    console.log(` ${r.team_name}: ${r.correct}/${r.total} correct, ${r.pick_pts} pts`),
  );
  ...
  console.log(
    ` Winner: ${m.winner_id ? (m.winner_id === (await db.query('SELECT id FROM league_members WHERE team_name=$1', [m.home])).rows[0]?.id ? m.home : m.away) : 'TBD'}`,
  );

  const perfect = await db.query(
    `SELECT lm.team_name, pcb.fights_correct, pcb.points_awarded FROM perfect_card_bonuses pcb JOIN league_members lm ON lm.id=pcb.member_id`,
  );
  console.log('\nPerfect card bonuses:', perfect.rows.length ? perfect.rows : 'none');

  const winBonus = await db.query(
    `SELECT lm.team_name, rwb.points_awarded FROM roster_win_bonuses rwb JOIN league_members lm ON lm.id=rwb.member_id`,
  );
